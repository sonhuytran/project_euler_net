 ...
 * 
 */
public class P11_LargestProductInAGrid {

	private static long largestHorizontal(int N, int[][] grid) {
		long largest = 1;
	...
	private static long largestVertical(int N, int[][] grid) {
		long largest = 1;
	...
	private static long largestDiagonal(int N, int[][] grid) {
		long largest = 1;
	...
	private static long largestReverseDiagonal(int N, int[][] grid) {
		long largest = 1;
	...
	public static void main(String[] args) {
		...
		int[][] grid = {
				...
				{ 1, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52, 1,
						89, 19, 67, 48 } };
		long maxProduct = largestVertical(N, grid);
		long maxHorizontal = largestHorizontal(N, grid);

		if (maxHorizontal > maxProduct) {
			maxProduct = maxHorizontal;
		}

		long maxDiagonal = largestDiagonal(N, grid);

		if (maxDiagonal > maxProduct) {
			maxProduct = maxDiagonal;
		}
		
		long maxReverseDiagonal = largestReverseDiagonal(N, grid);
		
		if (maxReverseDiagonal > maxProduct) {
			maxProduct = maxReverseDiagonal;
		}
